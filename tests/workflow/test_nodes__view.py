...
class Test(TestCase):  # noqa: D101
    ...
    def test_view_node_lifecycle(self):
        ...
        self.assertEqual(TestWorkflow.approve._owner_permission, 'viewflow.can_approve_process')
        ...
        self.assertEqual(response.status_code, 403, 'Should not allow to execute task before assign')
...
class TestWorkflow(flow.Flow):  # noqa: D101
    ...
    approve = (
        flow.View(
            views.UpdateProcessView.as_view(fields=[])
        ).Permission(
            auto_create=True
        ).Next(this.end)
    )
...
urlpatterns = [
    path('', FlowAppViewset(TestWorkflow).urls)
]

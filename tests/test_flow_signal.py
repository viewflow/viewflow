...
class IgnorableSignalFlow(Flow):
    start = flow.StartSignal(start_ignorable_test_signal, create_flow).Next(this.signal_task)
    signal_task = flow.Signal(ignorable_test_signal, IgnorableReceiver).Next(this.end)
    end = flow.End()

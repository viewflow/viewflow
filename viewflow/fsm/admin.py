...
class FlowAdminMixin(object):
    ...
    def create_revision(self, request):
        """Save wrapper to use in state transition views, in case of subclass of django-reversion VersionAdmin."""
    ...
    def render_change_form(self, request, context, add=False, change=False, form_url='', obj=None):
        if change:
            ...
            context.update({
                ...
                'flow': flow,
                'transitions': [
                    (transition, transition.conditions_met(flow), transition.has_perm(flow, request.user))
    ...
    def transition_view(self, request, object_id, slug):
        ...
        readonly_fields = [
            field for field in flatten_fieldsets(self.get_fieldsets(request, obj))
        ...
        return render(request, self.transition_form_template or [
            "admin/%s/%s/fsm_transition_form.html" % (opts.app_label, opts.model_name),

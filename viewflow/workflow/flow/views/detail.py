...
class UserIndexTaskView(generic.RedirectView):
    ...
    def get_redirect_url(self, *args, **kwargs):
        ...
        if activation.start.can_proceed() and flow_task.can_execute(self.request.user, task):
            return flow_task.reverse('execute', args=[task.process_id, task.pk])

        if activation.assign.can_proceed() and flow_task.can_assign(self.request.user, task):
        ...
        messages.success(self.request, _('You have no rights to view this task'), fail_silently=True)
